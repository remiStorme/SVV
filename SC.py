        ...
        if i == 0 or i == 2 or i == 3 or i == 5: #for sections with booms
            ...
            for j in range(len(s[i])):  # for all the discretized points in the segment
                if j < m:
                    ...
                    # print("i=", i, "j=", j, "m=", m)
                    qbs_i.append((-1 / I_zz * trapezoid(f[i], s[i])[
                        j - 1]) + shearjump)  # j-1 because trapezoid() leads to a list of values of areas that sum up to an integral, so number of elements in trapezoid is always 1 less than integrated function. qbsi[-1] adds the constant value of jump in shear due to the boom which is equal to the shear flow at the boom minus the one before
                    if bindxlst[i].index(m) != (
                            len(bindxlst[i]) - 1):  # if you didnt reach the last boom in the section yet
